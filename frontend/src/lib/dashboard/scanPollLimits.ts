...
/** HistoricalScanSelector UI: poll every 3s until the backend reports a terminal status (no client timeout alerts). */
export const HISTORICAL_SCAN_UI_POLL_INTERVAL_MS = 3_000;
export const HISTORICAL_SCAN_UI_POLL_MAX_ATTEMPTS = Math.ceil(
  HISTORICAL_GMAIL_SCAN_POLL_MAX_MS / HISTORICAL_SCAN_UI_POLL_INTERVAL_MS
);
