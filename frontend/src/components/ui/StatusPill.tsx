...
const toneStyles: Record<StatusPillTone, { color: string; backgroundColor: string; borderColor: string }> = {
  ...
  neutral: { color: colors.textSecondary, backgroundColor: colors.surface, borderColor: colors.border },
};

export function StatusPill({ tone, children }: { tone: StatusPillTone; children: ReactNode }) {
  return (
    <span
      className={`inline-flex min-w-11 items-center justify-center whitespace-nowrap border px-3 py-1 font-semibold ${radius.pill} ${type.body}`}
