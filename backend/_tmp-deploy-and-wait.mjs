...
const FRONTEND = process.env.RENDER_FRONTEND_SERVICE_ID || "srv-d898po77f7vs73bu01vg";
const headers = {
...
const backendDeploy = await triggerDeploy(BACKEND);
console.log(JSON.stringify({ backendDeployId: backendDeploy.id ?? backendDeploy.deploy?.id }, null, 2));
await waitLive(BACKEND, "backend");
...
console.log(JSON.stringify({ ok: true, backend: BACKEND, frontend: FRONTEND }, null, 2));
