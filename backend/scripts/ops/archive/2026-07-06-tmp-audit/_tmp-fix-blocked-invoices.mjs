...
for (const review of reviews) {
  if (review.gmailMessageId) {
    const invoices = await prisma.invoice.findMany({
      where: { gmailMessageId: review.gmailMessageId },
      select: { id: true, organizationId: true, status: true },
    });
    console.log("review", review.id, "invoices", invoices);
    for (const inv of invoices) {
      actions.push({ type: "delete_or_quarantine_invoice", invoiceId: inv.id, reviewId: review.id });
    }
  }
...
console.log(JSON.stringify({ actions, execute }, null, 2));
...
if (execute) {
  for (const action of actions) {
    ...
    if (action.type === "delete_or_quarantine_invoice") {
      await prisma.invoice.delete({ where: { id: action.invoiceId } });
    }
  }
