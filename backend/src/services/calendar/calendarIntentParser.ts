...
const DATE_TIME_BOUNDARY =
  /(?:^|\s)(?:ממחרתיים|ממחר|מהיום|מיום|למחרתיים|למחר|להיום|ליום|מחרתיים|מחר|היום|ביום|יום|בזמן|בשעה|ב-?\d|ב\s+\d|בבוקר|בערב|בצהריים|בצהרים|בלילה|ראשון|שני|שלישי|רביעי|חמישי|שישי|שבת|בשלושה|בשלוש|בארבעה|בארבע|בחמישה|בחמש|בשישה|בשש|בשבעה|בשבע|בשמונה|בתשעה|בתשע|בעשרה|בעשר|באחת|בשתיים|בשניים|\d{1,2}[:.]\d{2}|\d{1,2}[./]\d{1,2})/u;
...
const NEXT_READ_PATTERNS: RegExp[] = [
  ...
  // "מי/מה הלקוח הבא" (+ optional "שלי")
  /(?:מה|מי)\s+(?:ה)?(?:לקוח|לקוחה)\s+(?:ה)?(?:בא|הבא)(?:\s+שלי)?/u,
];
...
const LIST_PATTERNS: RegExp[] = [
  ...
  // "מתי התור/הפגישה של שרית?" / "מתי יש לי תור עם שרית?"
  /מתי\s+(?:ה)?(?:תור|פגישה)\s+של/u,
  /מה\s+(?:ה)?(?:תור|פגישה)\s+של/u,
  /מתי\s+(?:יש\s+לי\s+)?(?:תור|פגישה)\s+עם/u,
...
function detectListReadMode(text: string): {
  ...
  if (isNextReadIntent(text)) {
    if (/(?:מה|מי)\s+(?:ה)?(?:לקוח|לקוחה)\s+(?:ה)?(?:בא|הבא)/u.test(text)) {
