...
test("invoices-screen membership: camera draft stays in completion until confirm", async () => {
  ...
  const toCandidate = (row: {
    ...
    const assessment = assessInvoiceCompleteness({
      ...
      amountResolved: row.totalAmount != null,
      currency: row.currency,
      currencyExplicit: row.currency != null,
      date: row.documentDate,
      documentDateExplicit: row.documentDate != null,
  ...
  // Missing date/currency on an approved camera row stays in completion (FSE requires explicit date).
  ...
  assert.equal(filterInvoicesByCompleteness([withoutDate], "complete").length, 0, "approved camera without date stays in completion");
  assert.equal(filterInvoicesByCompleteness([withoutDate], "incomplete").length, 1, "missing date routes to completion queue");
});
...
test("confirm writes documentDate and currency so the approved row is invoices-screen complete", async () => {
  ...
  const confirmDb = {
    financialDocumentReview: {
      findFirst: async () => ({
        ...
        driveFileUrl: null,
        driveUploadStatus: null,
        parsedFieldsJson: { camera: { fileSha256: "f".repeat(64) } },
      }),
      update: async (args: any) => {
        updates.push(args);
        return { id: "draft-1" };
  ...
  const result = await confirmCameraDocument(
    ...
    { prismaClient: confirmDb, recordManualEntry: async () => ({ action: "accepted", payment: { id: "pay-9" } }) }
  );
  assert.equal(result.status, "approved");
  assert.ok(updates[0].data.documentDate instanceof Date, "documentDate must be written on approve");
  assert.equal(updates[0].data.currency, "ILS", "currency must be written on approve");
});
