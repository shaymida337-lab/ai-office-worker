...
export const CORE_INTEGRITY_CHECKS: readonly IntegrityCheckDefinition[] = [
  ...
  def("fin-fdr-payment-status-mismatch", "financial", "FDR needs_review with active payment", "Review and payment statuses diverged", "critical", true),
  def("fin-null-document-fingerprint", "financial", "Missing documentFingerprint", "Legacy payment without fingerprint", "warning", true),
  def("org-cross-org-reference", "organization", "Cross-org reference", "Entity references foreign organization data", "critical", true),
  def("org-cross-org-gmail-id", "organization", "Cross-org Gmail ID", "gmailMessageId appears in multiple organizations", "critical", true),
