  ...
  if (imageMimeType) {
    ...
    return {
      ...
      analysis: {
        ...
        amountBeforeVat: roundMoneyOrNull(result.amountBeforeVat),
        vatAmount: roundMoneyOrNull(result.vatAmount),
    ...
    try {
      const createResult = await createSupplierPaymentIfTrusted({
        ...
        data: {
          ...
          supplierTaxId: input.supplierMetadata.taxId,
          amountBeforeVat: roundMoneyOrNull(input.analysis.amountBeforeVat),
          vatAmount: roundMoneyOrNull(input.analysis.vatAmount),
          totalAmount,
          confidenceScore: input.classification.confidence,
