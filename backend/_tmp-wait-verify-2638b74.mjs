...
const COMMIT = process.env.VERIFY_COMMIT ?? "2638b74";
...
async function waitForLive(serviceId, label, timeoutMs = 15 * 60 * 1000) {
  const started = Date.now();
  while (Date.now() - started < timeoutMs) {
    const deploys = await listDeploys(serviceId);
    const match = deploys.find((d) => commitPrefix(d).startsWith(COMMIT.slice(0, 7)));
    ...
    console.log(
      JSON.stringify({
        ...
        lookingFor: COMMIT,
        latest: latest
          ? { id: latest.id, status: latest.status, commit: commitPrefix(latest), message: latest.commit?.message?.slice(0, 80) }
          : null,
        match: match
          ? { id: match.id, status: match.status, commit: commitPrefix(match) }
          : null,
      })
    );
...
async function verifyProd() {
  ...
  try {
    ...
    const ok =
      ...
      crm.activeCustomers === dashboard.active_clients &&
      crm.newLeads === dashboard.new_clients_this_month;
